...
while True:
    ...
    # video = np.uint8(frame2*256)
    video = np.round(frame2*256).astype(np.uint16).squeeze()
    # video = cv2.flip(video,0)
    cv2.imwrite('./test.png',video)
    # io.imsave('/content/drive/MyDrive/Road-Segmentation/test.jpg',video)
